<?php
/**
 * Update Schedule Script for Lanka Transit Admin
 * Handles schedule editing with validation and constraint enforcement
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            // Check if bus already has another schedule on this date
            $scheduleDate = date('Y-m-d', strtotime($departureDateTime));
            if ($schedule->hasScheduleOnDate($busId, $scheduleDate, $scheduleId)) {
                header('Location: schedule_listing.php?msg=This bus already has another schedule on ' . $scheduleDate . '. Each bus can only have one schedule per day.&type=error');
                exit();
            } else {
                // Update the schedule
                if ($schedule->updateSchedule($scheduleId, $busId, $departureDateTime, $arrivalDateTime, $fare)) {
                    header('Location: schedule_listing.php?msg=Schedule updated successfully!');
                    exit();
                } else {
                    header('Location: schedule_listing.php?msg=Failed to update schedule. Please try again.&type=error');
                    exit();
                }
            }
        } catch (Exception $e) {
            error_log("Schedule update error: " . $e->getMessage());
            header('Location: schedule_listing.php?msg=An error occurred while updating the schedule.&type=error');
            exit();
        }
    } else {
